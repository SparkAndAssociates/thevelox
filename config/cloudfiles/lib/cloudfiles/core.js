...
var Cloudfiles = exports.Cloudfiles = function (config) {
  if(config.authToken){
    var self = this;     
    self.config = config;
    self.config.cache = {path: path.join(__dirname, '..', '..', 'cache')};
    this.authorized = true;

    var cachePath = path.join(self.config.cache.path, self.config.auth.username);
...
Cloudfiles.prototype.setAuth = function (callback) {
  var authOptions = {
    uri: 'http://' + this.config.auth.host + '/v2.0/tokens',
    headers: {"content-type":"application/json"},
    body: JSON.stringify({auth: { tenantName: this.config.auth.tenantName, passwordCredentials:{username: this.config.auth.username, password: this.config.auth.password}}})
  };
  ...
  request.post(authOptions, function (err, res, body) {
    ...
    var result = JSON.parse(res.body).access;
    //self.swiftCert.authorized = true;
    //self.setStorageUrl(result.serviceCatalog[0].endpoints[0].publicURL);
    //self.swiftCert.authToken = result.token.id

    self.authorized = true;
//    self.config.serverUrl = res.headers['x-server-management-url'];
    self.config.setStorageUrl(result.serviceCatalog[0].endpoints[0].publicURL);
//    self.config.cdnUrl = res.headers['x-cdn-management-url'];
    self.config.authToken = result.token.id;
    self.config.id = result.user.id;
...
Cloudfiles.prototype.getFile = function (container, filename, res,  callback) {
  var self = this, 
...
Cloudfiles.prototype.addFile = function (options, callback) {
